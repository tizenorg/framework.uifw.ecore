...
_ecore_evas_x_event_client_message(void *data __UNUSED__, int type __UNUSED__, void *event)
{
   ...
   if (e->message_type == ECORE_X_ATOM_E_COMP_SYNC_BEGIN)
        ...
        if (ee->prop.wm_rot.supported)
          {
             ...
             ERR("ROT_PREPARE win:0x%08x, a:%d, configure_coming:%d, resize:%d (%dx%d)",
                 ee->prop.window, ee->prop.wm_rot.angle, ee->engine.x.wm_rot.configure_coming,
                 ee->prop.wm_rot.win_resize, ee->prop.wm_rot.w, ee->prop.wm_rot.h);
          }
     }
   else if (e->message_type == ECORE_X_ATOM_E_WINDOW_ROTATION_CHANGE_REQUEST)
     {
        ...
        if (ee->prop.wm_rot.supported)
          {
             ERR("ROT_REQ win:0x%08x", ee->prop.window);
             if (ee->prop.wm_rot.app_set)
               {
                  ...
                  if (!ee->engine.x.wm_rot.configure_coming)
                    {
                       ...
                       ERR("ROT_SET win:0x%08x, a:%d, resize:%d, configure_coming:%d (%dx%d)",
                       ...
                       if (ee->engine.x.wm_rot.configure_coming)
                         {
                            ...
                            ERR("SET MANUAL_RENDER win:0x%08x", ee->prop.window);
                         }
                    }
               }
             else
        ...
        if (ee->prop.wm_rot.supported)
          {
             if ((ee->prop.wm_rot.w == e->w) &&
                 (ee->prop.wm_rot.h == e->h))
               {
                  ...
                  ee->engine.x.wm_rot.configure_coming = 0;
                  ERR("CONFIGURE_NOTI win:0x%08x (%d x %d)",
