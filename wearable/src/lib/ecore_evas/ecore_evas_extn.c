   ...
   if (extn)
     {
        ...
        if (extn->animator)
          {
             ecore_animator_del(extn->animator);
             extn->animator = NULL;
          }
        free(extn);
        ecore_ipc_shutdown();
...
static Eina_Bool
_op_update_done_cb(void *data)
{
   Ecore_Evas *ee = data;
   Extn *extn;
   Ipc_Data_Update *ipc;

   if (!ee) return ECORE_CALLBACK_CANCEL;
   extn = ee->engine.buffer.data;
   if (!extn) return ECORE_CALLBACK_CANCEL;
   ...
   switch (e->minor)
     {
      ...
      case OP_UPDATE_DONE:
         // updates finished being sent - done now. frame ready
           {
              if (!extn->animator)
                extn->animator = ecore_animator_add(_op_update_done_cb, ee);
           }
         break;
      case OP_LOCK_FILE:
         if ((e->data) && (e->size > 0) &&
             (((unsigned char *)e->data)[e->size - 1] == 0))
           {
              ...
              extn->file.lockfd = open(extn->file.lock, O_RDWR);
           }
         break;
      case OP_SHM_REF:
         // e->ref == w
